...
- ホームは「ファイルを先に受け取り、拡張子から行き先の候補を出す」入口として扱う。
  - 拡張子と操作候補の対応は `models/quick-start-actions.js` だけに置く。componentのtemplateへ拡張子の分岐を書かない。
  - 判定はMIME typeではなく拡張子で行う。ドロップ時のMIME typeはOSとブラウザで揺れ、`FileTypeValidator` とBEの判定も拡張子のため。
  - 操作候補は担当タブへ移動するだけにし、変換や外部送信をホームのワンクリックで走らせない。
  - サイズ・種別の検証は移動先と同じ既存経路（`applyPdfFile` など）へ任せる。ホームだけ検証が緩い／二重に出る状態を作らない。
  - 受け付けられなかった場合はタブを移動しない。移動すると、移動先にファイルが無い理由が読めなくなる。
- 受け取ったファイルを担当タブへ渡して手放すcomponentでは、`event.target.value = ""` で選択欄も空へ戻す。
  - 値が残っていると、同じファイルを選び直しても `change` が発火せず、押しても何も起きない状態になる。
- ブラウザ内保存は `models/ui-preference-state.js` に集約する。pdf-app.jsはパスワードなど機密の画面状態も持つため、書き込み経路を増やさない。
  - 保存するのは表示設定・下書き・履歴のメタ情報だけにする。ファイルの中身は保存しない。
  - `File` はセキュリティ上パスを持たないため、名前を保存しても開き直せない。開き直せない履歴は押せるボタンにせず、文字のまま出す。
...
- ページ指定（削除・抽出）と分割範囲の入力は、判定と変換の両方で同じ `normalizePagesText`（`validation/page-number-validator.js`）を通す。
  - 判定と変換で処理が分かれると「入力欄では正しいと言われたのに送信でエラーになる」というズレが起きる。
- 吸収する表記ゆれは次のとおり。入力欄は1以上の整数・ハイフン・カンマだけを受け付けるため、記号の読み替えで誤解釈は起きない。
  - 全角英数字 → 半角（既存の `Util.toHalfWidth`）。**この関数の変換範囲は `[Ａ-Ｚａ-ｚ０-９]` の英数字だけで、記号は含まない。** 記号は次の2行で読み替える。
  - 全角カンマ・読点（`，` `、`）→ 半角カンマ。日本語入力で出やすく、`，`（U+FF0C）は `toHalfWidth` の範囲外。
  - 全角ハイフン・ダッシュ・長音（`－` `‐` `–` `—` `―` `ー`）→ 半角ハイフン。`－`（U+FF0D）以外は全角形の範囲外なので、`toHalfWidth` を広げても吸収できない。
  - 半角・全角スペース、タブ、改行 → 除去（`"2, 3-5"` のように区切りの後ろへ空白を入れる書き方を許容する）
- 空白除去の正規表現は `/\s+/g` を使う。`/\\s*|\t|\r|\n/` は正規表現リテラル中の `\\` がリテラルのバックスラッシュを意味するため、第1候補が「バックスラッシュ + 0個以上の `s`」になり空白と無関係。gフラグを足しても直らず、エスケープを直す必要がある。この形は `FrontendPageInputContractTest` で再混入を検知する。
- 入力欄のplaceholderは半角カンマの流儀にそろえる。受け付ける表記は広くとるが、案内は画面が生成する値（`1-3,5`）とAPIの形式に合わせる。**placeholderを変える場合は、その表記が正規化を通ることを確認する。**
- 画面側から送る値は既存APIの形式を変えない。ページ指定は `List<Integer>`、分割範囲は `List<String>`（`1-5` 形式）のまま。

## サムネイルからのページ選択

- サムネイルは **pdf.jsでブラウザ内に描く**。サーバーへは送らない。
  ...
  - PDFを画面で見るためだけにアップロードしない。パスワード付きPDFのパスワードも、サムネイル描画のためにブラウザの外へ出さない。
...
- pdf.jsはCDNから読まず、`static/vendor/pdfjs/` へバージョン固定で置く。
  - `build/` だけ差し替えない。日本語PDFの表示には `cmaps/` と `standard_fonts/` が要る。更新手順は同ディレクトリの `README.md`。
