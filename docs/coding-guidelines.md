...
- 吸収する表記ゆれは次のとおり。入力欄は1以上の整数・ハイフン・カンマだけを受け付けるため、記号の読み替えで誤解釈は起きない。
  - 全角英数字 → 半角（既存の `Util.toHalfWidth`）
  - 全角カンマ・読点（`，` `、`）→ 半角カンマ。**入力欄のplaceholderが全角カンマで案内している**ため必須。
  - 全角ハイフン・ダッシュ・長音（`－` `‐` `–` `—` `―` `ー`）→ 半角ハイフン。日本語入力では半角ハイフンより出やすい。
  - 半角・全角スペース、タブ、改行 → 除去（`"2, 3-5"` のように区切りの後ろへ空白を入れる書き方を許容する）
- 空白除去の正規表現は `/\s+/g` を使う。`/\\s*|\t|\r|\n/` は「バックスラッシュ + s」の並びを探すため空白にマッチせず、gフラグが無いと先頭の空マッチで置換が終わる。この形は `FrontendPageInputContractTest` で再混入を検知する。
