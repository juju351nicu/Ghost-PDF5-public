...
- ページ指定入力で空白と全角の区切り文字が扱えていなかった不具合を修正
  - `util.js` の `replaceBlank` の正規表現 `/\\s*|\t|\r|\n/` は「バックスラッシュ + s」の並びを探すため空白文字にマッチせず、gフラグも無いため先頭の空マッチで置換が終わっていた。空白が1つも除去されていなかった
  - このため `"2, 3-5"` のように区切りの後ろへ空白を入れた入力や、入力欄のplaceholderが案内している全角カンマ `"2，3-5"` が「整数の値を入力してください。」になっていた
  - 判定と変換で同じ `normalizePagesText` を通すようにし、全角英数字・全角カンマ/読点・全角ハイフン/長音・空白を吸収する。分割範囲入力も同じ経路にそろえた
  - `FrontendPageInputContractTest` で壊れた正規表現の再混入と、判定・変換の経路の分岐を検知
