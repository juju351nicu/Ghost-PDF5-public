...
- ページ指定入力で空白と全角の区切り文字が扱えていなかった不具合を修正
  - **原因1**: `util.js` の `replaceBlank` の正規表現 `/\\s*|\t|\r|\n/` が、そもそも空白を表していなかった。正規表現リテラル中の `\\` はリテラルのバックスラッシュのため、第1候補は「バックスラッシュ + 0個以上の `s`」で空白と無関係。空白しか含まない入力ではどの候補にもマッチせず置換が起きなかった（gフラグの有無ではなくエスケープの問題）
  - **原因2**: `toHalfWidth` の変換範囲が `[Ａ-Ｚａ-ｚ０-９]` の全角英数字だけで、全角カンマ `，`（U+FF0C）と全角ハイフン `－`（U+FF0D）が入っていなかった。全角数字は半角になるが区切り文字は全角のまま残っていた
  - このため `"2, 3-5"` のように区切りの後ろへ空白を入れた入力や、入力欄のplaceholderが案内していた全角カンマ `"2，3-5"` が「整数の値を入力してください。」になっていた
