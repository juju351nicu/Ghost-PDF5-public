 ...
 * 全角英数字を半角に変換する。
 *
 * 変換範囲は `[Ａ-Ｚａ-ｚ０-９]` の英数字だけで、記号は含まない。全角カンマ `，`（U+FF0C）や
 * 全角ハイフン `－`（U+FF0D）は変換されないため、ページ指定入力での記号の読み替えは
 * `validation/page-number-validator.js` の `normalizePagesText` が行う。
 * 読点 `、` や長音 `ー` は全角形の範囲外で、ここを全角記号へ広げても吸収できない。
 *
 * @param {string} target 文字列
 * @returns {string} 変換した文字列
 */
 ...
 *
 * 以前の正規表現 `/\\s*|\t|\r|\n/` は、そもそも空白を表す正規表現になっていなかった。
 * 正規表現リテラル中の `\\` はリテラルのバックスラッシュを意味するため、第1候補は
 * 「バックスラッシュ + 0個以上の `s`」であり、空白文字とは無関係。空白しか含まない入力では
 * どの候補にもマッチせず、置換が起きなかった。gフラグの有無ではなくエスケープが原因のため、
 * `\\s` を `\s` へ直す必要がある。
 *
 * @param {string} target 文字列
